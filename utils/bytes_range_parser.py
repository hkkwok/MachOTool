class BytesRangeParser(object):
    ...
    def add_subrange(self, data=None, length=None):
        ...
        br = self.bytes_range.add_subrange(self.current, length, data=data)
        self.current += length
        return br
    ...
    def add_subrange_beneath(self, data=None, length=None):
        ...
        return self.bytes_range.insert_subrange(self.start, length, data=data)

    def _get_bytes(self, length=None):
        if length is None:
            length = self.start + self.cmd_size - self.current
        return self.bytes_range.bytes(self.current, self.current + length)
